    ...
    const handleValidateButtonClick = (e: MouseEvent) => {
      ...
      formRef.value?.validate(async (errors) => {
        if (!errors) {
          ...
          try {
            const res = await signin<any>(username || '', password || '')
...
<template>
  <div class="h-full max-w-4  p-4 mx-auto" style="max-width: 360px;">
    ...
    <div class="flex mt-4">
      <div class="ml-auto ">
        <span>没有账号？</span>
        <a class="text-blue-500" href="javascript:" @click="handleRegister">注册账号</a>
      </div>
    </div>
  </div>
</template>
